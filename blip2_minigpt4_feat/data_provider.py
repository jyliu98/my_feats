...
class clipImageDataset(ImageDataset):
    def __init__(self, id_path_file, oversample=False, preprocess=None, existed_feat_file=''):
        exited_ids = []
        if os.path.exists(existed_feat_file):
            with open(existed_feat_file) as f:
                exited_ids = [line.split(' ', 1)[0] for line in f.readlines()]
        print('existed ids: %d'%len(exited_ids))
        
        exited_ids = set(exited_ids)
        data_ = list(map(str.strip, open(id_path_file).readlines()))
        data = [x for x in tqdm(data_) if x.split()[0] not in exited_ids]
        
        print("total ids:%d,  to process ids: %d"%(len(data_), len(data)))

        self.image_ids = [x.split()[0] for x in data]
        self.file_names = [x.split()[1] for x in data]
        self.preprocess = preprocess
        assert self.preprocess != None
