...
def process(options, collection):
    ...
    model_name1 = options.model_name1
    model_name2 = options.model_name2

    # layer = 'avgpool'
    batch_size = options.batch_size
    # feat_name = get_feature_name(model_name, layer, oversample)
    feat_name1 = get_feature_name(model_name1, oversample)
    feat_name2 = get_feature_name(model_name2, oversample)


    feat_dir1 = os.path.join(rootpath, collection, 'FeatureData', feat_name1)
    id_file1 = os.path.join(feat_dir1, 'id.txt')
    feat_file1 = os.path.join(feat_dir1, 'id.feature.txt')
    feat_dir2 = os.path.join(rootpath, collection, 'FeatureData', feat_name2)
    id_file2 = os.path.join(feat_dir2, 'id.txt')
    feat_file2 = os.path.join(feat_dir2, 'id.feature.txt')
    ...
    if options.split != "-1":
        ...
        feat_file1 += options.split
        print('id_path_file:%s \nfeature_file:%s' % (id_path_file, feat_file1))
        feat_file2 += options.split
        print('id_path_file:%s \nfeature_file:%s' % (id_path_file, feat_file2))

    if not os.path.exists(feat_dir1):
        os.makedirs(feat_dir1)

    if not os.path.exists(feat_dir2):
        os.makedirs(feat_dir2)

    for x in [id_file1, feat_file1, id_file2, feat_file2]:
        if os.path.exists(x):
    ...
    if options.resume:
        feat_file1 += 'resume'
        feat_file2 += 'resume'
    print("feat_file1: %s" % feat_file1)
    print("feat_file2: %s" % feat_file2)

    fw1 = open(feat_file1, 'w')
    fw2 = open(feat_file2, 'w')
    ...
    for image_ids, image_tensor in dataloder:
        ...
        output1 = torch.mean(output1, dim=1, keepdim=False)
        output2 = torch.mean(output2, dim=1, keepdim=False)
        target_feature1 = output1.cpu().numpy()
        target_feature2 = output2.cpu().numpy()
        for i, image_id in enumerate(image_ids):
            fw1.write('%s %s\n' % (image_id, ' '.join( ['%g' % (x) for x in target_feature1[i] ])))
            fw2.write('%s %s\n' % (image_id, ' '.join( ['%g' % (x) for x in target_feature2[i] ])))
...
def main(argv=None):
    ...
    parser.add_option("--model_name1",default=DEFAULT_MODEL_NAME, type="string")
    parser.add_option("--model_name2", default=DEFAULT_MODEL_NAME, type="string")
